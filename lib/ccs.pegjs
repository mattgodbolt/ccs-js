{
    ...
    function SelectorBranch(leaf, opt) {
        this.type = 'selector';
        this.leaf = new SelectorLeaf(opt ? 'descendant' : 'conjunction', [leaf]);
    }
...
Vals "constrain value" // TODO whitespace here?
  = "." head:Ident tail:("." Ident)* { return [head].concat(pickNth(tail, 1)); }
